...
export default function _27_UnsecuredLoansLiability({ ledgerData, onLedgerUpdate, selectedGroup, setFormStatus, setLedgerData, nameFormik, setCancelForm }) {
    const [currentLedgerData, setCurrentLedgerData] = useState(ledgerData);

    const formik = useFormik({
        initialValues: {
            name: nameFormik.values.name,
            statutory_include_value_calculation: currentLedgerData?.statutory_include_value_calculation || false,
            gst_applicability: currentLedgerData?.gst_applicability || false,
        ...
        onSubmit: async (values, { setErrors }) => {
            ...
            try {
                if (currentLedgerData) {
                    await axiosInstance.put(`/company/ledgers/unsecured-loans-liability/${currentLedgerData.id}/`, request_data);
                    ...
                    await axiosInstance.post('/company/ledgers/unsecured-loans-liability/', request_data);
        ...
        <>
            <form onSubmit={formik.handleSubmit}>
                <Grid container spacing={2}>
                    ...
                    <GridInputSelect
                        formik={formik}
                        name={'statutory_include_value_calculation'}
                        options={[
                            {
                                id: false,
                                name: 'Not Applicable'
