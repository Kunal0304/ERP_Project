...
export default function ReceiptVoucher() {
    ...
    const formikDefaultValues = {
        id: null,
        account: '',
        receipt_number: 'COMP/24/03/11/1',
        account_object_id: '',
            ...
            try {
                if (formStatus == 2) {
                    const data = {
                        ...
                        receipt_number: request_data.receipt_number,
                        account_object_id: request_data.account_object_id,
                        account_content_type: request_data.account_content_type,
                        from_party: request_data.records[0].from_party,
                        from_party_object_id: request_data.records[0].from_party_object_id,
                        from_party_content_type: request_data.records[0].from_party_content_type,
                        amount: request_data.records[0].amount,
                        narration: request_data.narration,
                        date: request_data.date,
                    }
                    await axiosInstance.put(`/voucher/receipt-voucher/${request_data.id}/`, data)
                    ...
                    request_data.records.forEach(async (record) => {
                        const data = {
                            ...
                            from_party_object_id: record.from_party_object_id,
                            from_party_content_type: record.from_party_content_type,
                            amount: record.amount,
                            narration: request_data.narration,
                            date: request_data.date,
                        }

                        await axiosInstance.post('/voucher/receipt-voucher/', data)
    ...
    return (
        <MainCard title={formStatus == 2 && 'Update Receipt Voucher' || formStatus && 'Add Receipt Voucher' || 'Receipt Vouchers'
                                                    ...
                                                    <TableRow key={index} sx={{ border: '1px solid #dddddd' }}>
                                                        ...
                                                        <TableCell width="73%" sx={{ borderLeft: '1px solid #dddddd' }}>
                                                            <GridAutoComplete
                                                                formik={formik}
                                                                getOptionLabel={option => option.name + ' - ' + option.id.charAt(0).toUpperCase() + option.id.split('_')[0].slice(1, -6) || ''}
                                                                getValue={allLedgers.find(ledger => ledger.id == formik.values.records[index].from_party_object_id) || null}
                                                                name={`records[${index}].from_party`}
                                                                label={'From Party'}
                                                                onChange={(event, option) => {
                                                                    formik.setFieldValue(`records[${index}].from_party`, '');
                                                                    formik.setFieldValue(`records[${index}].from_party_object_id`, option?.id || '');
                                                                    formik.setFieldValue(`records[${index}].from_party_content_type`, option?.content_type || '');
                                                    ...
                                                    <Avatar
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            formik.setFieldValue('records', [{
                                                                from_party: voucher.from_party,
                                                                from_party_object_id: voucher.from_party_object_id,
                                                                from_party_content_type: voucher.from_party_content_type,
