...
export default function SalesVoucher() {
    ...
    const fetchAllVouchers = () => {
        Promise.all([
            axiosInstance.get('/voucher/sales-regular-voucher/'),
            axiosInstance.get('/voucher/sales-deemed-export-voucher/'),
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setErrors }) => {
            ...
            try {
                if (formStatus == 2)
                    await axiosInstance.put(`/voucher/sales-${request_data.voucher_type}-voucher/${request_data.id}/`, request_data)
                else
                    await axiosInstance.post(`/voucher/sales-${request_data.voucher_type}-voucher/`, request_data)
                const generateEWayAndIncovie = confirm('Generate E-Way Bill & Invoice')
                if (generateEWayAndIncovie) {
                    alert('Geneated')
                }
    ...
    return (
        <MainCard title={formStatus && 'Add Sales Voucher' || 'Sales Vouchers'} formStatus={formStatus} setFormStatus={setFormStatus}>
            {
                formStatus ?
                    <>
                                                    ...
                                                    <Avatar
                                                        ...
                                                        onClick={() => {
                                                            if (confirm('Are you sure?')) {
                                                                (async () => {
                                                                    try {
                                                                        await axiosInstance.delete(`/voucher/sales-${voucher.voucher_type}-voucher/${voucher.id}/`)
