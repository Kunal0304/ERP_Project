...
export default function PurchaseOrderVoucher() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setErrors }) => {
            ...
            try {
                if (formStatus == 2)
                    await axiosInstance.put(`/voucher/purchase-order-voucher/${request_data.id}/`, request_data)
                else
    ...
    return (
        <MainCard title={formStatus && 'Add Purchase Order Voucher' || 'Purchase Order Vouchers'} formStatus={formStatus} setFormStatus={setFormStatus}>
            {
                formStatus ?
                    <>
                                                    ...
                                                    <Avatar
                                                        ...
                                                        onClick={() => {
                                                            if (confirm('Are you sure?')) {
                                                                (async () => {
                                                                    try {
                                                                        await axiosInstance.delete(`/voucher/purchase-order-voucher/${voucher.id}/`)
